...
if ($zip       !== '') { $sql .= " AND zip LIKE ?";       $params[] = '%'.$zip.'%'; }
if ($address1  !== '') { $sql .= " AND address1 LIKE ?";  $params[] = '%'.$address1.'%'; }
if ($mail      !== '') { $sql .= " AND mail LIKE ?";      $params[] = '%'.$mail.'%'; }
...
<div class="main">
  ...
  <div class="content">
    ...
    <div class="card">
      <form method="post" action="mail-work.php" onsubmit="return document.querySelector('input[name=\'send[]\']:checked') ? true : (alert('送信先を選択してください。'), false);">
        <div class="table-wrap">
          <table class="tbl">
            <thead>
              <tr>
                <th style="width:50px;" class="center">
                  <!-- check all button -->
                  <input type="checkbox" id="checkAll" onclick="document.querySelectorAll('input[name=\'send[]\']').forEach(el=>el.checked=this.checked)">
                </th>
                <th>コード</th>
                <th>顧客名</th>
                <th>フリガナ</th>
            ...
            <tbody>
              ...
              <tr>
                <td class="center"><input type="checkbox" name="send[]" value="<?= (int)$c['customer_id'] ?>"></td>
                <td><span class="badge badge-gray"><?= htmlspecialchars($c['customer_id'], ENT_QUOTES, 'UTF-8') ?></span></td>
                <td style="font-weight:600;"><?= htmlspecialchars($c['name'], ENT_QUOTES, 'UTF-8') ?></td>
                <td class="text-muted"><?= htmlspecialchars($c['kana'], ENT_QUOTES, 'UTF-8') ?></td>
                <td><?= htmlspecialchars($c['mail'], ENT_QUOTES, 'UTF-8') ?></td>
                <td><span class="badge badge-indigo"><?= htmlspecialchars($c['group'], ENT_QUOTES, 'UTF-8') ?></span></td>
              </tr>
              <?php endforeach; ?>
              <?php endif; ?>
            </tbody>
